...
**训练实践** 指的是一整套让[神经网络](../neural-network/)"训得动、训得稳、训得好"的工程技巧——从参数初始化、归一化、[正则化](../regularization/)，到学习率调度、早停和训练监控。它们各自简单，合起来却决定了一个模型能否真正可用。

**相关：** [Dropout](../dropout/) · [BatchNorm](../batchnorm/) · [早停](../early-stopping/)

## 直观理解

训练一个网络很像照料一锅在炖的汤：火太大（学习率太高）会糊，火太小（学习率太低）半天不熟；食材没洗净（数据没归一化）味道发苦；调料一次放太多（模型太复杂）会齁（[过拟合](../overfitting/)）。所谓训练实践，就是这套"火候 + 配料 + 尝味道"的经验集合——目标是让[损失](../backpropagation/)平稳下降、并且模型在**没见过的数据**上也表现好。
...
- **输入归一化**：把每个特征缩放到相近量级（如减均值除标准差），否则量级大的特征会主导[梯度](../gradient/)，训练扭曲。
- **层间归一化**：[BatchNorm](../batchnorm/) 在每个 [batch](../batch/) 上把层输入拉回零均值单位方差再缩放平移，稳定分布、允许更大学习率；小 batch 或序列任务里则常用 LayerNorm。
...
[学习率](../learning-rate/) 是最关键的超参数。配合[优化器](../optimizer/)（如 [Adam](../adam/)、SGD+Momentum）使用，常见技巧：
