...
矩阵乘法是[线性代数](../linear-algebra/)最核心的运算：它把一组向量做**线性变换**，也是把多个线性变换**串联**成一个的方式。[神经网络](../neural-network/)的每一层、[注意力机制](../attention/)，底层都是矩阵乘法。
...
- **批处理**：把一批样本堆成矩阵，一次乘法同时处理所有样本——这是 [GPU](../gpu/) 高效的关键。计算量约为 $O(mkn)$，所以模型规模直接决定算力需求。
