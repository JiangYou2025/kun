...
| 年代 | 人物 / 文明 | 探索（零散的数据或方法） |
|------|------------|--------------------------|
| ~前 1600 | 巴比伦 | **天文日记**：系统记录行星位置与日月食，用于历法与天象预测 |
| 古代起 | 古埃及 | **尼罗河水位计 (Nilometer)**：逐年记录洪水水位，跨千年的环境序列（农业/税收） |
| 前 800– | 中国史官 | **太阳黑子、彗星、日食、地震**的连续记录——现存最长的观测序列之一 |
| ~150 | Ptolemy | 《天文学大成》**本轮模型**：用周期叠加预测行星运动（确定性谐波模型） |
| 1213 | 中世纪欧洲 | **伦敦桥潮汐表**：最早的潮汐预测表之一 |
| 1662 | John Graunt | 《死亡数字观察》：首张**生命表**，分析伦敦死亡序列（统计学与人口学之始） |
| 1786 | William Playfair | 发明**折线图**：第一次把经济数据画成"随时间变化的曲线" |
| 1807/1822 | Fourier | **傅里叶级数 / 谐波分析**：把周期信号分解为正弦叠加 |
| 1805/1809 | Legendre · Gauss | **最小二乘**：用观测序列拟合天体轨道（谷神星） |
| 1843 | Schwabe | 发现约 **11 年的太阳黑子周期** |
| 1847 | Buys-Ballot | **周期检测表**：从序列中寻找隐藏周期 |
| 1848– | Wolf | **沃尔夫太阳黑子数**序列（后来正是 Yule 建模的对象） |
| 1862/1875 | Jevons · Juglar | **经济周期**、商业波动，乃至太阳黑子—景气假说 |
| 1876 | Kelvin (W. Thomson) | **潮汐预测机**：用谐波分析机械合成潮汐序列 |
| 1884 | Poynting | 用**移动平均**平滑价格序列、剥离趋势 |
| 1886 | Galton | **回归**与"回归到均值" |
| 1898 | Schuster | 发明**周期图 (periodogram)**：检验隐藏周期性——直通 20 世纪谱分析 |

> 这些工作大多是**观测先行、方法零散**：长期记录（天文、水文、人口、物价）积累了数据，谐波分析、最小二乘、移动平均、周期图等则给出最早的工具。它们在 1900 年后被随机过程理论统一，正式开启[上面的百年时间线](#时间线-timeline)。
