...
**批归一化（BatchNorm, Batch Normalization）** 是一种放在网络层之间的操作：它在每个小批量（mini-batch）上把某一层的输入"拉回"到均值 0、方差 1，再用两个可学习参数重新缩放和平移，从而让训练更稳定、收敛更快。
...
- 还带有轻微的正则化效果（因为每个样本的归一化依赖整个 batch，引入了一点噪声）。
