...
- 想要方法与公式：[时间序列入门讲义](../../course/)，其中 [第 2 讲 · 数据准备](../../course/02/)（切分、回测、防泄漏）、[第 4 讲 · 可视化](../../course/04/)（分析结果、看残差）和 [第 3 讲 · 线性模型](../../course/03/) 正是“衡量好坏”和“先打败基线”这两块。
- 想亲手体验预测：翻到下一篇 [一次成功的预测](../prediction-in-action/)——用炮兵射击游戏理解拟合与外推。
- 想直接上手一个现代模型：跳到 [使用 KUN](../kun/)。
</div>
