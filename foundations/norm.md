...
范数是衡量向量"**长度 / 大小**"的函数。同一个向量，用不同范数量出来的"长度"不同；机器学习里最常用 $\ell_1$ 与 $\ell_2$，它们也是正则化、距离度量的核心工具。
...
- **距离与相似度**：KNN、聚类用 $\ell_2$ 距离；误差度量 MSE 是 $\ell_2$、MAE 是 $\ell_1$。
- **梯度裁剪**：按梯度的 $\ell_2$ 范数缩放，防止[梯度](../gradient/)爆炸。

## 在时间序列预测中的意义

- 预测误差的度量本身就是范数：**MSE/RMSE = $\ell_2$**（对大误差敏感），**MAE = $\ell_1$**（对异常值更稳健）。选哪个取决于你更怕大错还是更怕被离群点带偏。
- 训练 KUN（Kernel U-Net）时加 weight decay（L2）有助于在长序列上稳住泛化；需要稀疏、可解释的特征时可考虑 L1。
