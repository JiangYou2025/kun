...
**反向传播（backpropagation）**是高效计算[损失函数](../loss-function/)对网络中**每一个权重的[梯度](../gradient/)**的算法。它本质上是**[链式法则](../chain-rule/)**在多层网络上的系统化应用：误差从输出层"倒着"一层层传回去，沿途告诉每个参数"你应该往哪个方向、调多少"。
...
- $\delta^{(l)}$：第 $l$ 层的误差信号（与该层[神经元](../neuron/)数同维）。
...
- **"反向传播 = [梯度下降](../gradient-descent/)"**——不是。反向传播只负责**算梯度**；怎么用梯度更新参数是[优化器](../optimizer/)的事。
