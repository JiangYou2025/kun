...
## 1. [神经网络](../neural-network/)的构成

- **[神经元](../neuron/)**：加权求和 + 偏置，再过一个非线性
- **层与网络**：神经元堆成层，层堆成网络
- **[前向传播](../forward-propagation/)**：输入逐层变换得到输出
- **[激活函数](../activation-function/)**：ReLU、Sigmoid、Tanh、GELU——引入**非线性**，否则整个网络退化成[线性模型](../linear-model/)

$$h = \sigma(Wx + b)$$

## 2. 如何训练：[反向传播](../backpropagation/)

- **[损失函数](../loss-function/)**：衡量预测与真实的差距
- **反向传播**：用**[链式法则](../chain-rule/)**把误差从输出层逐层传回，算出每个参数的梯度
- **[自动微分](../autodiff/)**：框架自动完成上述求导（你只写前向）
- **[优化器](../optimizer/)**：SGD、Momentum、RMSProp、**[Adam](../adam/)**（最常用）
- **[学习率](../learning-rate/)**：最关键的超参数，太大发散、太小太慢

## 3. 防止[过拟合](../overfitting/)

深度模型参数极多，泛化是核心难题。

- **[Dropout](../dropout/)**：训练时随机丢弃神经元
- **[批归一化 (BatchNorm)](../batchnorm/) / 层归一化 (LayerNorm)**：稳定训练
- **[早停 (early stopping)](../early-stopping/)**：验证集变差就停
- **数据增强 / 权重衰减**：进一步正则化

## 4. 常见[架构](../architectures/)
...
## 5. [训练实践](../training-practice/)

- **[批大小 (batch size)](../batch/)** 与 **轮次 (epoch)**
- **[GPU](../gpu/) 加速**：让大模型训练可行
