...
<div class="cards" markdown="0">
  ...
  <div class="card">
    ...
    <h3>鲁棒性 · Robustness</h3>
    <p>模型在干净的基准数据集上表现优秀，但到了真实世界就崩溃：数据缺失、传感器漂移、突发分布偏移（COVID 就是一次巨大的分布偏移）。如何让模型在"没见过的情况"下也不崩溃？对抗训练、域适应、重尾损失函数、在线学习是当前的主要武器。</p>
    <p style="color:var(--accent);font-size:.88rem"><strong>核心挑战：</strong>如何让模型优雅地应对"意料之外"？</p>
  </div>
</div>
...
| 鲁棒性 | 可插拔核函数（线性/MLP/注意力），适应不同数据特征 |
