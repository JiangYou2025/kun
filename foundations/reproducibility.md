...
- 权重的随机初始化、数据的随机打乱、[Dropout](../dropout/) 的随机失活；
- 不同版本的库（[NumPy](../numpy/) 1.24 vs 1.26）算出的数值可能有微小差异；
...
- **锁定环境**：记录 [Python](../python/) 版本和所有依赖的精确版本（`requirements.txt` / `environment.yml`），最好用 `venv`/`conda`/Docker 隔离。
...
- **判断真假改进**：你说新加的下采样模块更好，必须在**完全相同的种子、数据切分、[评估指标](../evaluation-metrics/)**下对比，否则分不清是真进步还是随机波动。多跑几个种子、报均值±标准差更可信。
